...
import {CleanUserInterface, CreateUserInterface} from "users/interfaces/users.dto";
...
export class UsersService {
    ...
    getCleanUser(user: User): CleanUserInterface {
    ...
    async create(data: CreateUserInterface): Promise<CleanUserInterface> {
        const createdUser = await this.usersRepository.create({...data, role: RoleEnum.user})
        await this.usersRepository.save(createdUser)
        return this.getCleanUser(createdUser)
    }

    async update(userId: number, data: object): Promise<CleanUserInterface> {
